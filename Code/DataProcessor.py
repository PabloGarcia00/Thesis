...
class DatasetProcessor:
    ...
    def _check_overlap(self):
        datasets = [self.train_set, self.val_set, self.test_set]

        def to_tuple(data: np.array) -> list:
            tf, target, _ = data.T
            tuples = list(zip(tf, target))
            return tuples

        for s1, s2 in combinations(datasets, 2):
            s1, s2 = to_tuple(s1), to_tuple(s2)
            s1, s2 = set(s1), set(s2)
            overlap = s1 & s2
            if overlap:
                return True
    ...
    def _distribute_tfs(self, pos_dict: dict) -> tuple:
        tfs = list(pos_dict.keys())
        random.shuffle(tfs)

        train, val, test = dict(), dict(), dict()

        def len_dict(a_dict: dict) -> int:
            length = len(list(chain.from_iterable(list(a_dict.values()))))
            return length

        train_n = int(len_dict(pos_dict) * self.train_test_ratio)
        val_n = int(train_n * 0.1)
        for tf in tfs:
            if len_dict(train) < train_n:
                train[tf] = pos_dict[tf]
            elif len_dict(val) < val_n:
                val[tf] = pos_dict[tf]
            else:
                test[tf] = pos_dict[tf]
        return train, val, test

    def _negative_samples_from_split(self, train: dict, val: dict, test: dict,
                                     neg_dict: dict) -> tuple:

        train_n, val_n, test_n = dict(), dict(), dict()

        coupled = [(train, train_n), (val, val_n), (test, test_n)]
        for pos, neg in coupled:
            for tf, targets in pos.items():
                neg_targets = neg_dict[tf]
                random.shuffle(neg_targets)
                n_targets = len(targets)
                neg[tf] = neg_targets[:n_targets]
        return train_n, val_n, test_n

    def split_binary_no_overlap(self, label_data: pd.DataFrame,
                                targets: list) -> None:
        pos_dict = self._label_data_to_dict(label_data)
        neg_dict = self._neg_dict_from_pos_dict(pos_dict, targets)

        train_p, val_p, test_p = self._distribute_tfs(pos_dict)
        train_n, val_n, test_n = self._negative_samples_from_split(
            train_p,
            val_p,
            test_p,
            neg_dict
        ...
        self.train_set = np.array(
            self._compile_data(train_p, 1) +
            self._compile_data(train_n, 0)
        )
        self.val_set = np.array(
            self._compile_data(val_p, 1) +
            self._compile_data(val_n, 0)
        )
        self.test_set = np.array(
            self._compile_data(test_p, 1) +
            self._compile_data(test_n, 0)
        )

    def _multiclass_distribute_tfs(self, pos_dicts: dict) -> tuple:
        train, val, test = dict(), dict(), dict()

        for key, pos_dict in pos_dicts.items():
            train[key], val[key], test[key] = self._distribute_tfs(pos_dict)
        return train, val, test

    def _multiclass_neg_from_split(self, train: dict, val: dict, test: dict,
                                   neg_dicts: dict) -> tuple:
        train_n, val_n, test_n = dict(), dict(), dict()

        for key in neg_dicts.keys():
            split = self._negative_samples_from_split(train[key], val[key],
                                                      test[key],
                                                      neg_dicts[key]
                                                      )
            train_n[key], val_n[key], test_n[key] = split

        return train_n, val_n, test_n

    def split_multiclass_no_overlap(self, label_data: pd.DataFrame,
                                targets: list) -> None:
        pos_dicts = self._multiclass_dataset_to_pos_dict(label_data)
        neg_dicts = self._multiclass_neg_dict(pos_dicts, targets)

        train_p, val_p, test_p = self._multiclass_distribute_tfs(pos_dicts)
        train_n, val_n, test_n = self._multiclass_neg_from_split(train_p,
                                                                 val_p,
                                                                 test_p,
                                                                 neg_dicts
                                                                 )
...
if __name__ == "__main__":
    ...
    print("any overlap:", processor._check_overlap())
    ...
    bin_alt = DatasetProcessor()
    bin_alt.split_binary_no_overlap(label_data, tg_list)
    print(bin_alt.train_set.shape)
    print(bin_alt.val_set.shape)
    print(bin_alt.test_set.shape)
    print("any overlap:", processor._check_overlap())

    mlc_alt = DatasetProcessor()
    mlc_alt.split_multiclass_no_overlap(mlc_labels, tg_list)
    print(mlc_alt.train_set.shape)
    print(mlc_alt.val_set.shape)
    print(mlc_alt.test_set.shape)
    print("any overlap:", processor._check_overlap())
