...
RUNS = 10
verbose = False
MEDIA = Path("/home/llan/Desktop/WUR/thesis2/media2/binary/go/experimental")
...
go_all = torch.load("GO/GO_tensor.pt", map_location=torch.device("cuda"), weights_only=True)
go_iea = torch.load("GO/go_wo_iea.pt", weights_only=True, map_location=torch.device("cuda"))
go_comp = torch.load("GO/go_wo_comp.pt", weights_only=True, map_location=torch.device("cuda"))
# go_random = torch.randint_like(go_all, 0, 2)
...
model_all = GO(100, 7247, 2, 200).to("cuda")
model_all.name = "GO_ALL"
model_iea = GO(100, go_iea.shape[1], 2, 200).to("cuda")
model_iea.name = "GO_IEA"
model_comp = GO(100, go_comp.shape[1], 2, 200).to("cuda")
model_comp.name = "GO_COMP"
models = [model_all, model_iea, model_comp]

go_tensors = {"go_all": (go_all, model_all), "go_iea": (go_iea, model_iea), "go_comp": (go_comp, model_comp)}


comb = product(list(idx_files.keys()), list(go_tensors.keys()))
# random_comb = product(list(idx_files.keys()), models)
...
train_stats = {"Dataset": [], "Model": [],"Tensor": [], "Run": [], "Epoch": [], "Batch": [], "Loss": []}
val_stats = {"Dataset": [], "Model": [],"Tensor": [], "Run": [], "Epoch": [], "Mean_loss": [], "Accuracy": []}
test_stats = {"Dataset": [], "Model": [], "Tensor": [], "Run": [], "TF": [], "Target": [], "Label": [], "Pred": []}

start = time.time()

for data_dir, tensor in comb:
    go_data = go_tensors[tensor][0]
    model = go_tensors[tensor][1]
    ...
    for run in range(RUNS):
        ...
        for epoch in range(EPOCHS):
            ...
            train_stats["Tensor"].extend([tensor] * l)
            ...
            val_stats["Tensor"].extend([tensor])
        ...
        test_stats["Tensor"].extend([tensor] * l)
